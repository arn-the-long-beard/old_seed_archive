...
use quote::quote;
use syn::{export::TokenStream2, punctuated::Iter, Attribute, Field, Fields, Ident, Type, Variant};

pub fn id_variant_snippets(variants: Iter<'_, Variant>) -> Vec<TokenStream2> {
    ...
    let snippets = variants.enumerate().map(|(i, variant)| {
        ...
        match fields {
            Fields::Unit => id_as_unit_variant(ident.clone()),
            Fields::Unnamed(fields) => id_as_tuple_variant(ident.clone(), fields.unnamed.iter()),
            Fields::Named(fields) => id_as_struct_variant(ident.clone(), fields.named.iter()),
...
fn id_as_unit_variant(ident: Ident) -> TokenStream2 {
    quote! {
        Self::#ident =>  { None }
    }
}
fn id_as_tuple_variant(ident: Ident, fields: Iter<'_, Field>) -> TokenStream2 {
    let field = fields
        .into_iter()
        .next()
    ...
    let token = if check_t32.to_string().eq("u32") || check_t32.to_string().eq("&u32") {
        quote! { Some(nested.to_string())}
    } else {
        quote! { nested.get_id_parameter() }
    };
...
fn id_as_struct_variant(ident: Ident, fields: Iter<'_, Field>) -> TokenStream2 {
    ...
    let format = build_id_return(structs_tuple);
    quote! {
        Self::#ident{#structs} => #format
    }
}

fn build_id_check() -> TokenStream2 {
    quote! {
    if id.is_empty() {
         None
    }
